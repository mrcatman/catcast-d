      ...
      <div class="box box--with-header support-page__tickets">
        <div class="box__header">
          ...
          <div class="box__header__title" v-else>
            ...
            <span class="support-page__category">{{currentTicketCategoryName}}</span>
          </div>
        ...
        <div class="box__inner" :class="{'support-page__ticket': currentTicket.visible, 'support-page__tickets__panel': addNewTicketPanel.visible, 'support-page__tickets__list': !addNewTicketPanel.visible}">
          <div v-if="addNewTicketPanel.visible">
            ...
            <c-autocomplete @getvalue = "(value) => {addNewTicketPanel.data.user_id = value.id}" autocomplete-key="id" autocomplete-value="username" url="users/autocomplete" v-model="autocompleteUser" :placeholder="$t('tickets.user')" />
            ...
            <c-checkbox v-model="addNewTicketPanel.data.is_important" :title="$t('tickets.is_important')" />
          </div>
              ...
              <c-infinite-scroll ref="messages_list" @scrollToTop="onMessagesScrollTop()" :loading="currentTicket.loadingMessages" class="support-page__ticket__messages">
                <div class="support-page__ticket__message" :class="{'support-page__ticket__message--unread': !message.is_read}" :key="$index" v-for="(message, $index) in currentTicket.messages.data">
                  <div class="support-page__ticket__message__header">
                    <div v-if="message.is_answer" class="support-page__ticket__message__author">{{$t('tickets.you')}}</div>
                    <a target="_blank" :href="'/users/'+currentTicket.data.user.id" v-else class="support-page__ticket__message__author">{{currentTicket.data.user.username}}</a>
          ...
          <div class="list-container" v-else>
            <div class="list-container__inner">
              ...
              <div class="list-item list-item--not-link list-item--without-picture" :key="ticket.id" v-for="ticket in tickets.data">
                <div class="list-item__left">
                  <div class="list-item__captions">
                    <a @click="showTicket(ticket)" class="list-item__title">{{getTitle(ticket)}} <c-tag v-if="ticket.unread_user_messages > 0">+{{ticket.unread_user_messages}}</c-tag></a>
                    <div class="list-item__under-title">{{$t('tickets.last_update')}}: <strong>{{formatPublishDate(ticket.updated_at, false)}}</strong>, {{$t('tickets.user')}}: <a target="_blank" :href="'/users/'+ticket.user.id"><strong>{{ticket.user.username}}</strong></a></div>
                  </div>
                </div>
                <div class="list-item__right">
                  {{$t('tickets.statuses.'+ticket.admin_status)}}
                </div>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    computed: {
      currentTicketCategoryName() {
        if (!this.currentTicket || !this.currentTicket.data) {
            return "";
        }
        let categoryId = parseInt(this.currentTicket.data.category);
        let category = this.categories.filter(category => category.id === categoryId)[0];
        if (!category) {
          return "";
        }
        console.log(category);
        return this.$t(category.text);
      },
      categoriesList() {
    ...
    methods: {
      ...
      async onMessagesScrollTop() {
        if (!this.currentTicket.loadingMessages) {
          if (this.currentTicket.currentMessagesPage < this.currentTicket.messages.last_page) {
            ...
            let messages = (await this.$api.get(`tickets/${this.currentTicket.data.id}?admin=1&page=${(this.currentTicket.currentMessagesPage + 1)}`)).data;
      ...
      sendMessage() {
        if (this.currentTicket.text.length > 0) {
          ...
          this.$axios.put(`tickets/${this.currentTicket.data.id}`, {is_answer: true, text: this.currentTicket.text}).then(res => {
            ...
            if (res.data.status) {
              ...
              if (this.currentPage === 1) {
                ...
                this.currentTicket.data.unread_user_messages = 0;
      ...
      async showTicket(ticket) {
        ...
        let messages = (await this.$api.get(`tickets/${ticket.id}?admin=1`)).data;
      ...
      async load() {
        this.loading = true;
        this.tickets = (await this.$api.get(`tickets?admin=1&page=${this.currentPage}`)).data.tickets;
        this.loading = false;
      },
      addTicket() {
        this.addNewTicketPanel.loading = true;
        let data = this.addNewTicketPanel.data;
        data.is_admin = true;
        this.$axios.post('tickets', data).then(res => {
    ...
    async mounted() {
      this.loading = true;
      let data = (await this.$api.get(`tickets?admin=1`));
      if (data.status) {
        this.tickets = data.data.tickets;
        this.categories = (await this.$api.get('tickets/categories')).data.categories;
      } else {
