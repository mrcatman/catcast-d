  ...
  <div class="search-page">
    <div class="search-page__params" @keydown="onKeyDown">
      ...
      <div class="search-page__tags">
        <c-tags-input :title="$t('records.search.tags')" v-model="searchData.tags"/>
      ...
      <div class="search-page__sort-container">
        <c-select :options="sortOptions" :title="$t('records.search.sort.heading')" v-model="searchData.order"/>
      </div>
      <div class="search-page__button-container">
        <c-button big @click="loadSearch()">{{$t('records.search.heading')}}</c-button>
      </div>
    </div>
    <div class="search-page__results">
      <RecordsList :title="searchTitle" :searchParams="searchParams"/>
    </div>
  </div>
...
<script>
  ...
  export default {
    computed: {
      searchTitle() {
        if (!this.lastRequest) {
          return this.$t('records.search.title');
        }
        return this.$t('channels.search.by_request')+' "'+this.lastRequest+'"';
      }
    },
    async asyncData({app}) {
      let tags = (await app.$api.get(`/records/tags`));
      return {
        tags: tags.data.tags
      };
    },
    ...
    head() {
      return {
        title: this.$t('records.search.page_title')
      }
    },
    mounted() {
    ...
    data() {
      return {
        sortOptions: [
          {
            name: this.$t('records.search.sort.new'),
            value: 'new'
          },
          {
            name: this.$t('records.search.sort.most_listened'),
            value: 'most_listened'
          },
          {
            name: this.$t('records.search.sort.best'),
            value: 'best'
          },
          {
            name: this.$t('records.search.sort.old'),
            value: 'old'
          }
        ],
