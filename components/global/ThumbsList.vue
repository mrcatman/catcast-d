    ...
    <div class="thumbs-list__heading-container">
      ...
      <c-row align="center" justify="between" class="thumbs-list__heading" v-if="config.title || config.search || config.filters || config.canChangeView">
        <c-col auto-width v-if="config.title">
          <component :is="config.buttonUrl ? 'nuxt-link' : 'span'" :to="config.buttonUrl" class="thumbs-list__heading__text" :class="{'thumbs-list__heading__text--link': config.buttonUrl}">
            {{config.title}}
            <c-icon class="thumbs-list__heading__text__arrow" v-if="config.buttonUrl" icon="chevron_right"></c-icon>
          </component>
        </c-col>
