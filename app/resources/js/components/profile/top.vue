                    ...
                    <div class="profile-page__user">
                        ...
                        <router-link :to="`/users/${user.id}`" class="profile-page__user__name">
                            ...
                            <div class="profile-page__status">
                                <span class="profile-page__status__text profile-page__status__text--empty"
                                      v-if="!statusText && me?.id === user.id && !editStatus.visible">{{ $t('profile.change_status') }}</span>
                                <span class="profile-page__status__text"
                                      v-if="statusText && !editStatus.visible">{{ statusText }}</span>
                                <c-button transparent icon-only icon="edit"
                                          v-if="me?.id === user.id && !editStatus.visible"
                                          @click="editStatus.visible = true"/>
                                <c-row v-if="editStatus.visible">
                                    <c-col>
                                        <c-input v-model="editStatus.data.status_text"></c-input>
                                    </c-col>
                                    <c-col auto-width>
                                        <c-button color="green" :loading="editStatus.loading" @click="saveStatus()">
                                            {{ $t('global.ok') }}
                                        </c-button>
                                    </c-col>
                                </c-row>
