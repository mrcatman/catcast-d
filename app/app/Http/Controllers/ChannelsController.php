...
class ChannelsController extends Controller {
    ...
    public function index() {
        ...
        return $channels->toResourceCollection();
    }
...
//            $radio_api->request($server, 'create_script', ['id' => $channel->id, 'port' => $port]);
    ...
    public function getDeleted() {
        return Channel::withTrashed()->whereNotNull('deleted_at')->where(['user_id' => auth()->user()->id])->get()->toResourceCollection();
    }

    public function getLeft() {
        $ids = UserChannelPermissions::withTrashed()->whereNotNull('deleted_at')->where(['user_id' => auth()->user()->id])->pluck('channel_id');
        return Channel::withTrashed()->whereIn('id', $ids)->get()->toResourceCollection();
    }
