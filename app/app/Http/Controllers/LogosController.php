...
class LogosController extends Controller {
    ...
    public function getForChannel($channel_id) {
        ...
        if ($channel->pictures_data['logo']) {
             ...
             if ($channel_logo && $channel_logo->id > 0) {
                 ...
                 if (!$already_in_list) {
                     // Not a saved row - it stands for the channel logo itself.
                     // Built as a Logo rather than a plain object so that it
                     // answers to every field LogoResource asks for.
                     $synthetic_logo = new Logo([
                     ...
                     $synthetic_logo->id = -1;
                     $synthetic_logo->is_channel_logo = true;
                     $synthetic_logo->setRelation('picture', $channel_logo);
                     $logos->prepend($synthetic_logo);
                 }
            }
            return $logos->toResourceCollection();
        } else {
    ...
    public function getActive($channel_id) {
        ...
        return ['logo' => $logo ? $logo->toResource() : null];
    }
