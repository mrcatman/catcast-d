...
<div class="checkbox" :class="{'checkbox--disabled': disabled}" ref="checkbox">
  <input :disabled="disabled" type="checkbox" style="display: none" v-model="checked" />
	<div class="checkbox__inner" @click="onClick">
		<div v-if="isSwitch" class="checkbox__switch" :class="{'checkbox__switch--active': checked}">
			<span class="checkbox__switch__handle"></span>
		</div>
		<div v-else class="checkbox__element" :class="{'checkbox__element--active': checked}">
			<span class="checkbox__element__tick" v-if="checked"><i class="fa fa-check"></i></span>
		</div>
...
const checked = defineModel<boolean>(false);

const checkboxRef = useTemplateRef('checkbox');

const setColor = () => {
  if (props.color) {
    checkboxRef.value.style.setProperty('--active-color', props.color);
  } else {
    checkboxRef.value.style.removeProperty('--active-color');
  }
}
