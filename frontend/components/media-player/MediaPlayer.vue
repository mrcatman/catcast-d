<template>
  <custom-colors class="media-player__container" :class="containerClasses" :colors-scheme="design.colors_scheme">
    ...
    <div class="media-player" @mousedown="onPlayerMouseDown" @dragstart="onPlayerDragStart"
         :id="'tv_player_'+channel.id" :class="playerClasses" @mousemove="hovered = true" @mouseenter="hovered = true"
         @mouseleave="hovered = false" ref="container">
...
-->
      ...
      <div class="media-player__big-play-button" v-show="waitingForFirstClick && isVideo && !error"
           @click="tryToPlay()">
      ...
      <div class="media-player__controls">
        <div class="media-player__controls__outer">
          ...
          <media-player-progress-bar v-if="isVOD" :media="media" :duration="duration" :progress="progress"
                                     @setTime="setTime"/>
          <div class="media-player__controls__inner">
            <div class="media-player__controls__left">
              ...
              <div class="media-player__volume">
                <media-player-button :icon="settings.muted ? 'volume_off' : 'volume_up'"
                                     @click="setMuted(!settings.muted)"
                                     :tooltip="settings.muted ? $t('player.muted.off') : $t('player.muted.on')"/>
            ...
            <div class="media-player__controls__right">
              ...
              <media-player-button v-if="isVideo" @click="setFullscreen(!settings.fullscreen)"
                                   :icon="settings.fullscreen ? 'fullscreen_exit' : 'fullscreen'"
                                   :tooltip="settings.fullscreen ? $t('player.fullscreen.off') : $t('player.fullscreen.on')"/>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  watch: {
    ...
    playing(playing) {
      this.$store.commit('players/SET_IS_PLAYING', {
  ...
  data() {
    return {
      ...
      settings: {
        source: null,
        muted: false,
        volume: localStorage.player_volume ? parseInt(localStorage.player_volume) : 75, // todo: move to subcomponent maybe
        quality: localStorage.preferred_quality ? parseInt(localStorage.preferred_quality) : null,
        fullscreen: false,
        playbackRate: 1,
      }
    }
  ...
  methods: {
    ...
    initVOD() {
      let file = this.media.files.filter(file => file.quality === this.settings.quality)[0];
      if (!file) {
      ...
      return new Promise(resolve => {
        ...
        this.player.ready(() => {
          ...
          this.player.on('error', e => {
            ...
            if (error.code === 2 && !this.isVOD) { // live stream started, but HLS manifest not created yet
