...
export default {
  ...
  computed: {
    isBig() {
      let width = (this.circleWidth ? (typeof this.circleWidth !== "number" ? parseFloat(this.circleWidth) * 16 : this.circleWidth) : 16);
      return width > 36;
    },
    circleStyle() {
      let width = (this.circleWidth ? (typeof this.circleWidth !== "number" ? this.circleWidth : this.circleWidth + 'px') : '1em');
      let height = width;
      return {width:width,height:height}
    },
    calculatedSize () {
      return (this.circleWidth ?  Number(this.circleWidth) : 75) + (this.button ? 8 : 0)
    },
    circumference () {
    ...
    strokeDashOffset () {
	//console.log(((100 - this.normalizedValue) / 100) * this.circumference + 'px');
