...
export default {
  ...
  computed: {
    ...
    url() {
      return `/channels/?my=1&with_permissions=1${this.channelType !== '' ? '&type=' + this.channelType : ''}`;
    },
    channelTypes() {
      const types = [];
      if (this.allowedChannelTypes[CHANNEL_TYPE_TV] && this.allowedChannelTypes[CHANNEL_TYPE_RADIO]) {
        types.push({ id: CHANNEL_TYPE_ALL, name: this.$t('global.all')});
      }
      if (this.allowedChannelTypes[CHANNEL_TYPE_TV]) {
        types.push({ id: CHANNEL_TYPE_TV, name: this.$t('channels.tv')});
      }
      if (this.allowedChannelTypes[CHANNEL_TYPE_RADIO]) {
        types.push({ id: CHANNEL_TYPE_RADIO, name: this.$t('channels.radio')});
      }
      return types;
    }
  },
  ...
  methods: {
    ...
    getPermissionsList(channel) {
      return Object.keys(channel.permissions).map(key =>
        this.$t(`channel_permissions.${key}.name`)
      );
    },
  }
};
